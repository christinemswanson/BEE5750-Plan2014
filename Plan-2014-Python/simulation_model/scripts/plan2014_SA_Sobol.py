...
def my_PLAN2014_fun(c11,c12): # add more params to change...so far, just changing c1 params 
    ...
    wd = "C:/Users/cms549/Documents/GitHub/BEE5750-Plan2014/Plan-2014-Python/simulation_model"
    os.chdir(wd)
    ...
    filelist = os.listdir(os.getcwd() + "/input/" + expName + "/hydro")
    ...
    for f in range(startf, len(filelist)):
        ...
        for p in range(nseeds):
            ...
            if expName != "climate_scenarios":
                spinup = pd.read_table(
                    "input/" + expName + "/spin_up/" + filelist[f]
                )
            ...
            for t in range(s, timesteps):
                ...
                # historic version of M limit to prevent too low of flows
                    ...
                    if ontLevel > 74.20:
                        ...
                        if ontLevel >= (adj + 69.474):
                            mq2 = slope * (ontLevel - adj - 69.474) ** 1.5 # pre project release 
                        else:
                            mq2 = np.nan
                        mq = min(mq1, mq2)
                ...
                saunderstwLevel = round(
                    44.50
                    + 0.006338 * pow((data["saunderstwR"][t] * ontFlow * 10), 0.7158),
                    2,
                )
                ...
                summerstownLevel = round(
                    46.10
                    + 0.0109 * pow((data["summerstownR"][t] * ontFlow * 10), 0.451),
                    2,
                )
...
# Perform analysis
Si = sobol.analyze(problem, test_array, print_to_console=True)
