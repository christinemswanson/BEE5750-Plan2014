...
# os.chdir("/Users/kylasemmendinger/Box/Plan_2014/simulation_model")

# # import objective functions
# data = pd.read_csv("optimTimeSeries.txt", sep="\t")
# data = pd.read_csv("stochasticPlan2014.txt", sep="\t")
...
def coastalImpacts(
    ...
    # return upstreamImpacts, downstreamImpacts
    return (
        ontarioDamages,
        alexbayDamages,
        cardinalDamages,
...
ontarioLoadingCostDatabase = pd.read_csv("data/cnOntarioLoadingCosts.csv")
ontarioLoadingCostDatabase = {
    x: ontarioLoadingCostDatabase[x].values for x in ontarioLoadingCostDatabase
}

seawayLoadingCostDatabase = pd.read_csv("data/cnSeawayLoadingCosts.csv")
seawayLoadingCostDatabase = {
    x: seawayLoadingCostDatabase[x].values for x in seawayLoadingCostDatabase
}

montrealLoadingCostDatabase = pd.read_csv("data/cnMontrealLoadingCosts.csv")
...
def commercialNavigation(
    ...
    # return float(totalTransportationCost)
    return float(ontarioTotalCost), float(seawayTotalCost), float(montrealTotalCost)
...
mmAreaContours = pd.read_csv("data/mmContourAreas.csv")
...
def meadowMarsh(levels, nts, contours):
            ...
            if counter["ontNTS"].min() < lowT:
                ...
                if counter.shape[0] >= 4:
                    ...
                    if all(counter.loc[:3, "ontNTS"] < highT):
                        ...
                        # see if upper threshold is exceeded again, only take years to there
                        ...
                        # if threshold isn't exceeded again, take the rest of the years
                        else:

                            # find first occurence of average nts below the threshold
                            yearEnd = int(counter.loc[(counter.shape[0] - 1), "Year"])
    ...
    totalArea["mmLowSupply"] = 0
    totalArea.loc[(totalArea["Year"].isin(yearInd)), "mmLowSupply"] = 1
    # lowSupply = totalArea.loc[(totalArea["Year"].isin(yearInd))]

    # get meadow marsh in low supply years
    # lowSupply = lowSupply.loc[
    #     lowSupply["Classification"] == "BC", ["Year", "Area"]
    # ].reset_index(drop=True)
    totalArea = totalArea.loc[
        totalArea["Classification"] == "BC", ["Year", "Area", "mmLowSupply"]
    ].reset_index(drop=True)

    # return lowSupply, totalArea
    return totalArea
...
# -----------------------------------------------------------------------------
# evaluate objective functions over simulation output
# -----------------------------------------------------------------------------

# returns annual average of each objective
def objectiveSimulation(data):
    ...
    # hydro = dict((k, data[k]) for k in keys if k in data)

    # hydro["hydroValue"] =
    hydro[
        [
            "opgMosesSaundersEnergy",
            "opgMosesSaundersEnergyValue",
            "nypaMosesSaundersEnergy",
    ...
    # mm, totalArea = meadowMarsh(mmLevels, mmNTS, mmAreaContours)
